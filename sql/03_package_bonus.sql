-- Package with types and procedure demonstrating:
-- - RECORD types
-- - Associative arrays (index-by) and nested tables
-- - Use of GOTO for a small skip-case

CREATE OR REPLACE PACKAGE bonus_pkg IS
  -- Record to represent an employee row
  ...
  -- Associative array (index-by) of employee records grouped by dept
  TYPE t_emp_assoc IS TABLE OF t_employee_rec INDEX BY BINARY_INTEGER;

  -- Nested table to collect bonus report rows
...
CREATE OR REPLACE PACKAGE BODY bonus_pkg IS
  ...
  -- Shared nested table for produced bonuses
  ...
  PROCEDURE calculate_and_store_bonuses(p_bonus_pct IN NUMBER) IS
    ...
    -- associative array to temporarily hold employees of a department
  ...
  BEGIN
    ...
    LOOP
      ...
      -- Example use of RECORD (mapping cursor row to our package record)
      DECLARE
      ...
      BEGIN
        ...
        -- Small controlled use of GOTO:
        -- If employee is on leave, we skip bonus calculation and mark them as skipped.
        IF v_emp.on_leave = 'Y' THEN
          -- jump to SKIP_EMPLOYEE label to handle skipping logic
          GOTO SKIP_EMPLOYEE;
        END IF;

        -- Calculate bonus: simple percentage of salary, but minimum 100
